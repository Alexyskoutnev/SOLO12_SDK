...
main(int argc, char **argv)
{
	...
	for (int i = 0; i < N_SLAVES_CONTROLED; i++) {
		...
		//robot_if.motor_drivers[i].motor1->set_kp(0.);
		//robot_if.motor_drivers[i].motor2->set_kp(0.);
		//robot_if.motor_drivers[i].motor1->set_kd(0.);
		//robot_if.motor_drivers[i].motor2->set_kd(0.);
	...
	while (!robot_if.IsTimeout()) {
		if (((std::chrono::duration<double>)(std::chrono::system_clock::now() - last))
			...
			cpt++;
			t += dt;
			robot_if.ParseSensorData(); // This will read the last incomming packet and
			...
			case 0: // check the end of calibration (are the all controlled motor
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * 2; i++) {
					...
					init_pos[i] =
					    robot_if.motors[i].GetPosition(); // initial position
			...
			case 1:
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * 2; i++) {
					if (i % 2 == 0) {
						...
						if (robot_if.motor_drivers[i / 2].error_code ==
						    0xf) {
					...
					if (robot_if.motors[i].IsEnabled()) {
						...
						double v_ref = 2. * M_PI * freq * amplitude *
						    cos(2 * M_PI * freq * t);
						double p_err =
						    ref - robot_if.motors[i].GetPosition();
						double v_err =
						    v_ref - robot_if.motors[i].GetVelocity();
						double cur = kp * p_err + kd * v_err;
						if (cur > iq_sat)
							cur = iq_sat;
						if (cur < -iq_sat)
							cur = -iq_sat;
						robot_if.motors[i].SetCurrentReference(.1);
					}
				}
				break;
			}
	...
	printf("Masterboard timeout detected. Either the masterboard has been shut down or there "
	       "has been a connection issue with the cable/wifi.\n");
	return 0;
}
